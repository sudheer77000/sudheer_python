...
Three client teams - Arka Finance, BlueLeaf Retail, and CityRide Mobility - are migrating contracts, policies, support and  operational  reports  into  a  single  RAG  platform.  Each  team  has  different  document  types,  access  rules,  and  pars challenges. The RAG system must answer questions with citations while ensuring that one client never sees another clie data.

This  PDF  is  intentionally  designed  to  test  document  loaders,  PDF  parsers,  OCR  workflows,  table  extraction,  chunk strategies, metadata preservation, and source citation quality.
...
The platform team is building a document intelligence layer for contract operations. Legal users need answers such as: W contracts  renew  automatically?  Which  agreements  have  a  60-day  termination  notice?  Which  documents  mention  data residency in India? These questions require retrieval across multiple document types and sometimes across multiple rel documents of a single client.
...
|-------------------|----------------------------------|--------|----------------------|---------------------------------------------|
| Arka Finance      | MSA, audit addendum, DPA         | High   | Legal and compliance | What audit rights exist in the              |
...
The ingestion team receives files from multiple sources. Some files are clean and digital; others contain scanned p rotated tables, missing metadata, embedded images, or formula-generated values. The inventory below intentionally inclu different formats and extraction expectations.
...
| CRM-OPS-005    | CityRide MobilityCSV |                     | 48,200 rows  | Incident records             | Needs column type normalization          |
| CRM-FORM-00 6  | CityRide             | MobilityScanned PDF | 4 pages      | OCR fields, checkbox         | Low contrast and slight rotation         |
...
Clause 7.2 - Confidential Information. The Receiving Party shall protect Confidential Information using at least th degree of care that it uses to protect its own confidential materials, but in no event less than reasonable care. Co Information includes technical documents, pricing schedules, user lists, API keys, operational procedures, security r incident response notes, product roadmaps, and any derived analysis prepared by the Receiving Party.

Clause  7.3  -  Exclusions.  Confidential  Information  does  not  include  information  that  is  publicly  available  without independently developed without reference to the Disclosing Party material, received lawfully from a third party, or a for release in writing. The burden of proving an exclusion remains with the Receiving Party.

Clause 9.1 - Data Residency. If the Statement of Work identifies a specific processing region, the service provide process production records only within that region unless the client approves a transfer in writing. Disaster recovery may be stored in a secondary region if encryption at rest and access logging are enabled.

Clause 11.4 - Termination Assistance. Upon termination, the provider shall make client data available for export for a of thirty days. After the export period, the provider may delete production data according to the deletion schedule legal hold applies.

|   Clause | Short name             | Owner            | Trigger                          | Required action                                                      | Citation hint           |
|----------|------------------------|------------------|----------------------------------|----------------------------------------------------------------------|-------------------------|
|      7.2 | Confidentiality        | Receiving Party  | Receipt of confidential material | Protect information with reasonable and equivalent internal controls | care Page 4, clause 7.2 |
...
The following table is intentionally simple. It should be correctly extracted by most PDF table parsers. It tests basi column detection, short text values, and numeric values.
...
| Incident Response | Critical incidents require acknowledgement within minutes. | 30 SRE           | After each incident |
...
For RAG systems, simple tables are often converted into row-wise text chunks such as: Policy Area: Refunds; Rule: Refu requests must be raised within 7 days; Owner: Customer Support.

## 5. Complex Table: SLA, Escalation, and Credits

This table uses long cells and operational footnotes. It is designed to test whether the parser can retain header hiera map values to the correct columns.
...
| Data Pipeline      | P1 - ingestion stopped    | 20 minutes        | 6 hours             | 8 percent pipeline credit     | fee Queue depth, failed job IDs, replay report |
...
The diagram below represents the ingestion and retrieval flow. Some PDF parsers ignore images completely, while other extract image metadata but not the text inside the image. For production use, image content may require OCR or multimod extraction.
...
|-----------------|--------------------------------|-----------------------------------|
| Box labels      | OCR or vision model            | Raw Files, Parser, Chunks, Vector |
...
2. Re-run parser with `safe\_mode=True`.
...
This page contains a scanned-form style image. Text inside the form is not normal selectable PDF text. A simple text p may miss it entirely. OCR-based systems should extract labels, values, and checkboxes from the image.
...
BlueLeaf  Retail  has  five  related  contract  documents.  If  these  documents  are  indexed  independently  without  relation metadata,  the  retriever  may  miss  cross-document  context.  The  platform  uses  a  contract\_group\_id  to  connect  relate documents.
...
| BLR-PRICE-004 | Pricing Amendment        | BLR-ACME-2026       | 2026-05-01       | Updated pricing and discount   |
...
In a multi-tenant RAG system, each document, chunk, embedding, and retrieval request should be associated with a tenan identifier. Client isolation should happen before the LLM receives any context.
...
Wrong flow: retrieve from all clients and tell the model to ignore unauthorized content. This is unsafe because the LLM not be used as the access-control boundary.

## 11. Analytics Image: Retention Heatmap

The heatmap below simulates viewership retention by time slot. A parser that ignores chart images will miss useful bus context. OCR can extract axis labels and numbers, but chart understanding may require a vision model.
...
|------------------------------------|-------------------------|--------------------------------------------|
| Prime Friday performs best         | 93 retention score      | Place premium content in Friday late/prime |
...
A  RAG  system  should  be  evaluated  separately  for  retrieval  quality  and  answer  quality.  This  page  includes  a  miniat evaluation set with expected source references. It is useful for testing whether citations point to the correct section.
...
Evaluation should include adversarial questions, unrelated questions, and questions that require multi-hop retrieval related documents.

## 13. Edge Cases for Parsers

The following cases often break real document ingestion pipelines. They are included here as guidance for testing pa quality before moving to embeddings and vector storage.
...
Use this checklist before sending parsed content into chunking and embeddings. It helps identify whether the data is rea production RAG.
...
Grouped clauses with owner/backup split inside the same responsibility cell. This page is useful for testin grouping, merged-looking labels, split responsibility cells, and long evidence text.
...
|-----------------|--------------------------------------------------------|--------------------|--------------------|----------------------------------------|----------------------------------------|----------|
| Data Protection | termination urless retention i5 legally required.      | Backup             | Leqal              | Terminaticn noice recened              | Delebon cerificate audit export        | High     |
| Data Protection |                                                        | Backup             | Secunily           | Incider: classifed as corfirmed breach | notifcation prcof                      | Critical |
| Billing         | Apply annual platfom fee adjusiment only after renewal | Backup             |                    | Renewa] crder approved                 | Apfroved renewal sheet                 | Medium   |
| Billing         | during                                                 | Backup             | Revsue Ops Finance | Campus status suspended                |                                        | High     |
| Support         | Minujes hcurs. during                                  | Backup             |                    | Ticke: pricriy = P1                    | Ticket times amps ager: assignment Icg | High     |
| Support         | Escalate unresolved P2 tickets                         | Own Backup         |                    | business hours                         | Escalation bg                          | Medium   |
...
Pricing table with multi-level headers, regional columns, add-on columns, billing rules, exception rows, an mixed numeric/text values.
...
|------------|-------------------|-----------------------------------|-----------------------------------|-----------------------------------|----------------------------------|----------------------------------|----------------------------|
|            |                   | India Region                      | India Region                      | Interatonal                       | SMS                              | WhatsAPp                         |                            |
| Starer     | 0                 | Dase                              | INR 4 5L INR 6DK                  | USD } 2c0                         |                                  | INR Oa2message                   | Quarterly advance          |
| Growth     | 2,001             |                                   | INR 11L INR 14L                   |                                   |                                  | INR O38message                   |                            |
| Enterprise | 10,601 - sudents  | Base                              | Cussn Inzlded                     | Custcm                            | Negotiated                       | Negotialed                       | Signed ordar form required |
| Excepion   | Govanment schools | Disccunt may apply after apprcval | Disccunt may apply after apprcval | Disccunt may apply after apprcval | No discoun: Dn pass-through cost | No discoun: Dn pass-through cost | Requires CFO approval      |
...
Invoice-style line item table with item groups, quantity, rate, CGST/SGST split, totals, and summary row. U for invoice parsing and tax extraction tests.

Parsing challenge: preserve row boundaries, nested headers, split cells, grouped labels, numeric values, and footnotes/context around the table.

|         |             | Rale         | Tax Splll    | Tax Splll    | Tolal         |
|---------|-------------|--------------|--------------|--------------|---------------|
|         | 12 campises |              |              | 93           | INR 12,9B,DDI |
|         |             |              |              | 93           | INR 89,6BD    |
|         |             | INR 2,{0,ODD |              | 93           |               |
| sunnery |             |              | INR 1,21,440 | INR 1,27,440 | INR 16,7D,B@  |
...
| Client   |   Open | Critical   |             |
|----------|--------|------------|-------------|
| Arka     |     17 |            | Legal       |
| BlueLeaf |     22 |            | Procurement |
| CityRide |     16 |            | Ops         |
...
Synthetic scanned tax invoice with vendor details, customer details, invoice number, GSTIN, line items, tax s total amount, payment terms, footer notes, approval stamp, and handwritten-style receipt text.
...
School360 Learning Services Pvt Ltd Tower B, Outer Road Bengaluru, Karnataka 560103 Ring
...
Due within 15 Late fee may apply after due date. Footer note: Amount includes taxes unless separately mentioned. OCR challenge: faint stamp; rotated page; table lines, and handwritten approval. days.
...
Synthetic scanned utility bill with meter-style charges, usage rows, tax values, total payable amount, paymen terms, stamp, handwritten-style note, and noisy/rotated scan effects.

Parsing challenge: extract bill number, issuer, line items, usage quantity, tax split, total payable, and payment notes from scanned image.
...
| Energy usage peak units     |   842 | INR 8.20  |    6,904 |
| Energy usage off-peak units |   313 | INR 5.70  |    1,784 |
...
Total AmountINR 12,128

## Payment Terms:

Due within 15 days. Late fee may after due date Footer note: Amount includes taxes unless separately mentioned OCR challenge: faint stamp; rotated page, table lines, and handwritten approval. apply
